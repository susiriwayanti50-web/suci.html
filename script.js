...
// CONVERT FILE MEDIA LOKAL KE BASE64
...
async function tambahProduk(e) {
  ...
  const fileGambarInput = document.getElementById('fileGambar').files[0];
  const fileVideoInput = document.getElementById('fileVideo').files[0];
  const deskripsi = document.getElementById('deskripsiProduk').value;

  if (!fileGambarInput || !fileVideoInput) {
    alert("Silakan pilih file foto dan video terlebih dahulu!");
    return;
  }

  try {
    showToast("Mengunggah file media...");
    
    // Membaca file gambar dan video menjadi format Base64
    const gambarBase64 = await readFileAsBase64(fileGambarInput);
    const videoBase64 = await readFileAsBase64(fileVideoInput);
